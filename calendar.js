...
export function assignUsersToCalendar(month, year, users, options = {}) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (isTeamDay(day)) {
      ...
      const selectedUser = hasEnoughUsers
        ? availableUsers[0]
        : { name: "NOT SET" }; // Get the first available user
      calendar[day] = [
        selectedUser.name,
        "Team",
        { isValidDay: true, isAssigned: hasEnoughUsers },
      ]; // Assign user and "Team" for the second slot
      ...
      if (hasEnoughUsers && selectedUsers[0].name === selectedUsers[1].name) {
        displayError(
          `Some dataset duplication that should not happen. Please reach out to the code maintainer.`,
        );
...
export function generatePDF(calendar, month, year, usersData = []) {
  ...
  // Add a title
  doc.setFontSize(18);
  doc.text("Elterndienstplan", doc.internal.pageSize.getWidth() / 2, 20, {
    align: "center",
  });
  ...
  for (const day in calendar) {
    ...
    const isUser2InDataSet =
      usersData.find((user) => user === parent2) || parent2 == "Team";
    ...
    if (
      ...
      const confirmedChoice = confirm(
        `User nicht im Datenset gefunden an der Stelle: ${parent1}, ${parent2}. Trotzdem fortfahren?`,
      );
    ...
    rows.push({
      data: [dayParent1, dateParent1, parent1Content, parent2],
      meta,
    }); // Only 4 columns now
  }
