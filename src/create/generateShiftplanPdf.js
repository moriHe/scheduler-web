...
function getHeaders(isKita, cols) {
    if (isKita || cols === 2) {
        ...
        return [['Tag', 'Datum', schicht1, schicht2]]
    }
    ...
    return [['Tag', 'Datum', schicht1, schicht2, schicht3]];
}
    ...
    for (const day in calendar) {
        ...
        if (!actionTaken) {
            ...
            if (isValidDay && !areAllUsersKnown) {
                ...
                const confirmedChoice = confirm(`${text.create.personNotFoundOne} ${parents.join(", ")}. ${text.create.personNotFoundTwo}`);
                if (!confirmedChoice) {
                    return
                }
                actionTaken = true;
