...
function uploadUsers() {
    ...
    if (!file) {
        alert(getText("options.chooseFile"));
        return;
    }
    ...
    reader.onload = function (event) {
        try {
            ...
            if (uniqueUsers.size !== users.length) {
                alert(getText("options.duplicates"));
            ...
            alert(getText("options.uploadSuccess"));
        } catch (error) {
            console.log(error)
            alert(getText("options.notValidJson"));
        }
    };
...
function resetUsers() {
    const confirmReset = confirm(getText("options.revert"));
    if (confirmReset) {
        localStorage.setItem(getStorageKey(), JSON.stringify([])); // Reset users to an empty array
        alert(getText("options.revertSuccess")); // Optional: Alert to confirm reset
    }
}
